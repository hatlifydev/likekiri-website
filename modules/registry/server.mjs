...
/**
 * Servidor del módulo registry: manifest firmado + estáticos federados.
 * Escucha solo en loopback; Caddy lo expone bajo /modules/registry/.
 */

const PORT = Number(process.env.MODULE_PORT ?? 4003);
  ...
  menu: [
    {
      ...
      icon: 'server',
      order: 20,
      // Submenú plegable: demuestra el modo toggle del contrato.
      mode: 'toggle',
      children: [{ label: 'Registry de módulos', path: '/registry', order: 1 }],
    },
  ],
  permissions: [{ key: 'registry.read', label: 'Ver el registry de módulos' }],
};
